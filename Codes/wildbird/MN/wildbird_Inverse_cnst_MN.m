...
a = min(tdata);
b = max(tdata);
N0 = (A1 + A2)/2 + (A2 - A1)/2 * sin((pi/8)*a + 4*pi/3-t0);
Ntdata = (A1 + A2)/2 + (A2 - A1)/2 * sin((pi/8)*tdata + 4*pi/3-t0);
sigma_0 = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
theta0 = [sigma_0;3;1];
...
for j = 1:m
    ...
    title(sprintf('$%s = %.3f$ \n $(%.3f, %.3f)$', param_names{j}, mean_val, ci_95(1), ci_95(2)), 'Interpreter', 'latex', 'FontSize', font_main);
    set(gca, 'FontSize', font_axis, 'TickLabelInterpreter', 'latex');
end
...
Nprime = (A2 - A1)/2 * (pi/8) * cos((pi/8)*t  + 4*pi/3-t0);
N_a = (A1 + A2)/2;
dydt = zeros(2,1);
